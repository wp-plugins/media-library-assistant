...
class MLAReferences {
	...
	public static function mla_attachment_array_fetch_references_handler( &$attachments ) {
		...
		if ( MLACore::$process_mla_gallery_in ) {
			$have_mla_galleries = self::_build_mla_galleries( MLACore::MLA_MLA_GALLERY_IN_TUNING, self::$mla_galleries, '[mla_gallery', $exclude_revisions ); // TODO FIX THIS
		} else {
			$have_mla_galleries = false;
		}

		if ( MLACore::$process_gallery_in ) {
			$have_galleries = self::_build_mla_galleries( MLACore::MLA_GALLERY_IN_TUNING, self::$galleries, '[gallery', $exclude_revisions ); // TODO FIX THIS
		} else {
			$have_mla_galleries = false;
		}
